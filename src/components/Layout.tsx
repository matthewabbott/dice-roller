    ...
    return (
        <div className="min-h-screen bg-brand-background text-brand-text flex flex-col">
            ...
            <main className="flex-grow flex h-0">
                ...
                <div className={`flex-grow min-w-0 flex flex-col transition-all duration-300 ${isRightSidebarCollapsed ? 'w-full' : ''
                    }`}>
                    {/* Square Canvas - Main focus */}
                    <div className="flex-grow flex items-center justify-center p-4">
                        <div className="aspect-square w-full max-w-3xl relative">
                            <DiceCanvas />
                    ...
                    {/* Local Controls Panel - Below Canvas */}
                ...
                {/* Right Sidebar - Lobby (collapsible) */}
                <div className={`border-l border-brand-surface transition-all duration-300 ${isRightSidebarCollapsed ? 'w-0 overflow-hidden' : 'w-[25%] min-w-0'
                    }`}>
                    {/* Collapse Toggle Button */}
                    <button
                        onClick={() => setIsRightSidebarCollapsed(!isRightSidebarCollapsed)}
                        className="absolute right-0 top-1/2 transform translate-x-full -translate-y-1/2 bg-brand-surface hover:bg-brand-background border border-brand-surface rounded-r-md p-2 z-10 transition-colors"
                        title={isRightSidebarCollapsed ? "Expand lobby panel" : "Collapse lobby panel"}
                    >
